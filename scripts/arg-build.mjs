...
async function keyFor(answers, label, fixed = null) {
  const raw = fixed ? Buffer.from(fixed, 'base64') : crypto.getRandomValues(new Uint8Array(32));
  ...
  return { key, raw: b64(raw), wraps: wraps.sort(() => Math.random() - 0.5) };
}

/** keys: { n: base64 key } kept from one build to the next, so players keep their progress. */
export async function build(chapters, keys = {}) {
  let prevKey = null;
  const out = [];
  for (const [i, c] of chapters.entries()) {
    const n = i + 1;
    const { key, raw, wraps } = await keyFor(c.answers, n, keys[n]);
    keys[n] = raw;
    out.push({
      n,
...
<head>
  ...
  <style>
    ...
    .tuto { margin: 12px 0 4px; }
    .tuto summary { cursor: pointer; opacity: .85; text-decoration: underline dotted; }
    .tuto h3 { margin: 14px 0 4px; font: 700 15px 'Courier New', monospace; color: #b4ffe0; }
    .tuto ol { margin: 0; padding-left: 22px; }
    .tuto li { margin: 3px 0; }
  </style>
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const keysFile = path.join(root, 'arg', 'keys.json');
  const keys = fs.existsSync(keysFile) ? JSON.parse(fs.readFileSync(keysFile, 'utf8')) : {};
  const data = await build(CHAPTERS, keys);
  fs.writeFileSync(keysFile, JSON.stringify(keys, null, 2));
  fs.writeFileSync(path.join(root, 'public', 'js', 'arg-data.js'), `// Generated by scripts/arg-build.mjs — do not edit.\nexport default ${JSON.stringify(data)};\n`);
  const bedrock = config.MC_BEDROCK?.host ? config.MC_BEDROCK : null;
  const relay = await buildRelay(CHAPTERS[3].answers, { address: config.MC_ADDRESS, version: config.MC_VERSION, bedrock });
